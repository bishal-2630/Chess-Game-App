...
// import 'package:mqtt_client/mqtt_server_client.dart'; // REMOVED for Web compatibility
import 'mqtt_client_factory.dart'; // Added factory
import 'isolate_handler.dart'; // Added isolate handler
...
class MqttService {
  ...
  MqttClient? client; // Changed type to generic MqttClient
  ...
  Future<void> initialize() async {
    ...
    // AudioContextAndroid is not available on Web
    if (!kIsWeb && isMainIsolate) {
      try {
        ...
        await AudioPlayer.global.setAudioContext(audioContext).catchError((e) {
          AppLogger.w('⚠️ MQTT: Global audio context error: $e');
        });
      } catch (e) {
        AppLogger.w('⚠️ MQTT: Global audio context error: $e');
      }
  ...
  void initializeIsolateListener({bool isBackground = false}) {
    ...
    if (_isPortListening) {
      AppLogger.w('⚠️ MQTT: Isolate port already being listened to, skipping');
      return;
    }
    ...
    _listenerPort.listen((message) async {
      if (message == 'stop_audio') {
        ...
        if (message['action'] == 'stop_audio') {
          ...
          // Notification cleanup handled by screen or other isolates
        }
      }
    });
  }
      ...
      try {
        ...
        if (type == 'call_invitation' || type == 'incoming_call') {
          if (response.actionId == 'decline_action' || response.actionId == 'decline') {
            ...
            AppLogger.i('✅ [FG] ACTION: ACCEPT CALL');
        ...
        if (type == 'invitation_response') {
          ...
          if (action == 'accept' || response.actionId == 'join_action') {
            AppLogger.i('✅ [FG] ACTION: JOIN CHALLENGE (CONFIRMED)');
            _emitNotification({
              ...data,
        ...
        AppLogger.e('❌ [FG] Error: $e');
      }
    } else {
      AppLogger.w('⚠️ [FG] Notification payload is NULL');
    }
  }
  ...
  Future<void> connect(String username) async {
    if (isConnected) {
      AppLogger.w('⚠️ MQTT: Already connected, skipping');
      return;
    }
    ...
    client = setupMqttClient(broker, clientIdentifier, port);
    ...
    try {
      ...
      AppLogger.e('❌ MQTT: Connection failed - $e');
      disconnect();
      return;
    }
    ...
    if (client!.connectionStatus!.state == MqttConnectionState.connected) {
      ...
      AppLogger.e('MQTT: Connection failed - state is ${client!.connectionStatus!.state}');
      disconnect();
    }
  }
  ...
  void _listen() {
    if (_isListening) {
      AppLogger.w('⚠️ MQTT: Already listening, skipping');
      return;
    }
  ...
  Future<void> dismissCallNotification() async {
    try {
      ...
      AppLogger.e('Error dismissing call notification: $e');
    }
  }
  ...
  Future<void> cancelCallNotification({String? roomId, bool broadcast = true}) async {
    ...
    try {
      ...
      AppLogger.e('Notification cancellation error: $e');
    }
  ...
  Future<void> speakRecordingAnnouncement() async {
    try {
      ...
      await _flutterTts.speak("This call is being recorded for future use.");
    } catch (e) {
      AppLogger.e('TTS: Error speaking announcement: $e');
    }
  }
  ...
  Future<void> playSound(String fileName, {String? roomId}) async {
    ...
    if (_isMutedWindow) {
      AppLogger.d('MQTT [$isolateName]: Aborting playSound - Muted window');
      return;
    }

    // Safety check 1: already declined?
    if (roomId != null && _declinedRoomIds.contains(roomId)) {
      AppLogger.d('MQTT [$isolateName]: Blocking playSound - room $roomId already declined');
      return;
    }

    // Safety check 2: already in call?
    if (_isInCall) {
      AppLogger.d('MQTT [$isolateName]: Blocking playSound - user is in active call');
      return;
    }
    ...
    try {
      ...
      if (_isMutedWindow || _isInCall || (roomId != null && _declinedRoomIds.contains(roomId))) {
        AppLogger.d('MQTT [$isolateName]: Play blocked. Muted: $_isMutedWindow, InCall: $_isInCall, Declined: ${roomId != null && _declinedRoomIds.contains(roomId)}');
        _isPlaying = false;
      ...
      await _audioPlayer.setReleaseMode(ReleaseMode.loop).catchError((e) {
        AppLogger.e('MQTT [$isolateName]: Error setting release mode: $e');
      });
      await _audioPlayer.setVolume(1.0).catchError((e) {
        AppLogger.e('MQTT [$isolateName]: Error setting volume: $e');
      });
