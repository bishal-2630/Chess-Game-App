...
class DjangoAuthService extends ChangeNotifier {
  ...
  Future<void> initialize({bool autoConnectMqtt = true}) async {
    ...
    if (userData != null) {
      try {
        ...
        AppLogger.e('❌ [Auth] Error decoding stored user: $e');
      }
    }
    ...
    if (kIsWeb) {
      ...
      if (hasUrlTokens || _currentUser == null) {
        AppLogger.i('🌐 [Initialize] Starting web bootstrap (hasUrlTokens=$hasUrlTokens)...');
  ...
  Future<bool> _handleUrlTokens() async {
    try {
      ...
      AppLogger.d('🌐 [URL-Bridge] Checking tokens in: $currentUrl');
      ...
      if (access != null && refresh != null) {
        AppLogger.i('✅ [URL-Bridge] Captured tokens from QueryParams');
        _accessToken = access;
        _refreshToken = refresh;
        if (next != null) {
          _nextRoute = next;
          AppLogger.i('✅ [URL-Bridge] Captured next route: $_nextRoute');
        }
        await _saveAuthData();
        return true;
      }
      ...
      if (fragment.isNotEmpty) {
        AppLogger.d('🌐 [URL-Bridge] Parsing hash fragment: $fragment');
        ...
        if (fAccess != null && fRefresh != null) {
          AppLogger.i('✅ [URL-Bridge] Captured tokens from Fragment');
      ...
      AppLogger.e('❌ [URL-Bridge] Error: $e');
    }
    return false;
  }
  ...
  Future<void> _bootstrapWebSession() async {
    try {
      ...
      if (_accessToken != null) {
        ...
        AppLogger.d('🌐 [Bootstrap] Using JWT: ${_accessToken!.substring(0, 10)}...');
      } else {
        AppLogger.d('🌐 [Bootstrap] No JWT found, relying on session cookies');
      }

      late http.Response response;
      if (kIsWeb) {
        AppLogger.d('🌐 [Bootstrap] Using BrowserClient (withCredentials=true)');
  ...
  Future<void> signOut() async {
    try {
      ...
      try {
        ...
        AppLogger.e('🍪 Sign out error clearing cookies: $e');
      }
      ...
      try {
        ...
        AppLogger.e('📧 Sign out error clearing google session: $e');
      }
  ...
  Future<http.Response> authenticatedRequest(
    ...
    if (response.statusCode == 401 && _refreshToken != null) {
      AppLogger.w('🔄 [Auth] 401 detected. Attempting token refresh...');
      final success = await refreshToken();
      if (success && _accessToken != null) {
        AppLogger.i('🔄 [Auth] Refresh successful. Retrying request...');
  ...
  Future<bool> refreshToken() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        AppLogger.e('❌ Token refresh failed: ${response.body}');
